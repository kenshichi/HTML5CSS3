...
def translate_segments_to_vietnamese(segments: list, source_language: str, mode: str) -> Tuple[list, str]:
    ...
    try:
        ...
        try:
            ...
            translated = texts
            warning = "[WARNING] Chưa có mô hình dịch, giữ nguyên văn bản gốc."

    for j, idx in enumerate(pending_idx):
        vi = cleanup_vietnamese_text(translated[j])
        vi_sub = format_subtitle_text(vi)
        ...
        rows[idx]["translation_status"] = "Giữ nguyên văn bản gốc" if vi == rows[idx].get("source_text", "") else "Đã dịch"
    return rows, warning
