...
import subprocess
from src.videotranslatorpro.core.voice_registry import VOICES, as_display_items
from src.videotranslatorpro.core.tts_manager import TTSManager
...
def list_voices(provider: str = "edge-tts") -> List[Dict[str, str]]:
    voices = as_display_items()
    try:
        edge = _provider(provider).list_voices()
...
def synthesize_text(text: str, output_path: str, voice: str, speed: int = 0, pitch: int = 0, volume: int = 0, provider: str = "edge-tts") -> str:
    ...
    for v in VOICES:
        if v["id"] == voice:
            preferred = v.get("engine", "edge_tts")
            if preferred == "edge-tts":
                preferred = "edge_tts"
            break
    res = _manager.synthesize(text=text, voice=voice, output_path=output_path, speed=speed, pitch=pitch, volume=volume, preferred_engine=preferred)
