...
from src.videotranslatorpro.core.diarization import build_speaker_profiles, ensure_speakers, ManualDiarizationProvider, PyannoteDiarizationProvider, map_diarization_to_transcript, save_diarization_output
from src.videotranslatorpro.core.transcribe import transcribe_audio, save_transcript_json
from src.videotranslatorpro.core.subtitle_detection import detect_burned_in_subtitles
...
from src.videotranslatorpro.core.preview import create_preview
from src.videotranslatorpro.core.exporter import export_final_project
from src.videotranslatorpro.core.project_state import ProjectState, save_project_state, load_project_state, missing_cache_files
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.state = {"has_transcript": False, "has_translation": False, "has_subtitle": False, "has_subtitled_video": False, "has_tts": False, "has_dubbed_video": False, "has_subtitle_detection": False, "source_subtitle_detected": "unknown", "source_subtitle_position": "unknown", "subtitle_strategy_suggested": "Tự động, khuyên dùng", "subtitle_strategy_user_override": "", "tts_stale": False, "has_preview": False, "last_preview_path": ""}
    ...
    def _top_goal_bar(self) -> QWidget:
        ...
        g.addWidget(QLabel("Ở chế độ tự động, ứng dụng sẽ cố gắng phát hiện phụ đề gốc và chọn vị trí phù hợp."), 2, 0, 1, 5)
    ...
    def _page_goal(self) -> QWidget:
        ...
        self.tts_hint = QLabel("Edge TTS cần kết nối internet để tạo giọng đọc tiếng Việt.")
        ...
        v.addWidget(self.quick_hint); v.addWidget(self.tts_hint); v.addWidget(self.adv_group); v.addStretch(1)
        return w
    ...
    def _page_analysis(self) -> QWidget:
        ...
        self.btn_open_samples = QPushButton("Mở ảnh mẫu")
        self.subtitle_preview.setMinimumHeight(120)
        self.preview_help = QLabel("Preview giúp kiểm tra vị trí phụ đề, chất lượng bản dịch và giọng đọc trước khi xuất toàn bộ video.")
        self.preview_start = QLineEdit("00:00:00")
        self.preview_duration = QSpinBox(); self.preview_duration.setRange(5, 120); self.preview_duration.setValue(15)
        self.preview_type = QComboBox(); self.preview_type.addItems(["Preview phụ đề", "Preview lồng tiếng", "Preview phụ đề + lồng tiếng"])
        self.btn_make_preview = QPushButton("Tạo preview")
        self.btn_open_preview_file = QPushButton("Mở file preview"); self.btn_open_preview_file.setEnabled(False)
        self.btn_open_preview_folder = QPushButton("Mở thư mục preview")
        ...
        v.addWidget(self.btn_open_samples)
        v.addWidget(self.preview_help)
        fprev = QFormLayout(); fprev.addRow("Thời điểm bắt đầu preview", self.preview_start); fprev.addRow("Thời lượng preview (giây)", self.preview_duration); fprev.addRow("Loại preview", self.preview_type); v.addLayout(fprev)
        v.addWidget(self.btn_make_preview); v.addWidget(self.btn_open_preview_file); v.addWidget(self.btn_open_preview_folder)
        return w
    ...
    def _page_review(self) -> QWidget:
        ...
        self.speaker_mode = QComboBox(); self.speaker_mode.addItems(["Gán thủ công, khuyên dùng", "Tách tự động nếu khả dụng"])
        self.speaker_status = QLabel("Gán thủ công đang được dùng.")
        self.speaker_help = QLabel("Tách người nói tự động có thể sai. Vui lòng kiểm tra lại người nói trước khi tạo giọng đọc.")
        self.btn_generate_tts = QPushButton("Tạo giọng đọc")
        self.btn_auto_diarization = QPushButton("Tách người nói tự động")
        v.addWidget(self.speaker_table); v.addWidget(self.speaker_mode); v.addWidget(self.speaker_status); v.addWidget(self.speaker_help); v.addWidget(self.btn_auto_voice); v.addWidget(self.btn_apply_all); v.addWidget(self.btn_rename); v.addWidget(self.btn_preview_default); v.addWidget(self.btn_generate_tts); v.addWidget(self.btn_auto_diarization)
    ...
    def _page_export(self) -> QWidget:
        w = QWidget(); v = QVBoxLayout(w)
        self.btn_export_final = QPushButton("Xuất video hoàn chỉnh")
        self.btn_export_sub = QPushButton("Xuất phụ đề SRT/ASS")
        self.btn_render_video = QPushButton("Kết xuất video phụ đề")
        self.btn_export_dub = QPushButton("Xuất audio lồng tiếng")
        self.btn_save_project = QPushButton("Lưu dự án")
        self.btn_load_project = QPushButton("Mở dự án")
        self.btn_rerun_step = QPushButton("Làm lại bước này")
        self.btn_clear_cache = QPushButton("Xóa cache dự án")
        self.btn_open_output = QPushButton("Mở thư mục xuất")
        self.btn_report = QPushButton("Xuất báo cáo lỗi")
        self.export_summary = QLabel("Tóm tắt xuất file: --")
        self.progress = QProgressBar(); self.progress.setValue(0)
        self.progress_task = QLabel("Sẵn sàng")
        self.progress_detail = QLabel("--")
        self.progress_elapsed = QLabel("Thời gian: 0s")
        for b in [self.btn_export_final, self.btn_export_sub, self.btn_render_video, self.btn_export_dub, self.btn_save_project, self.btn_load_project, self.btn_rerun_step, self.btn_clear_cache, self.btn_open_output, self.btn_report]:
            b.setMinimumHeight(38); v.addWidget(b)
        v.addWidget(self.export_summary); v.addWidget(self.progress); v.addWidget(self.progress_task); v.addWidget(self.progress_detail); v.addWidget(self.progress_elapsed); v.addStretch(1)
        return w
    ...
    def _bind_events(self) -> None:
        ...
        self.btn_open_samples.clicked.connect(self._open_sample_frames)
        self.btn_make_preview.clicked.connect(self._create_preview_clip)
        self.btn_open_preview_file.clicked.connect(self._open_preview_file)
        self.btn_open_preview_folder.clicked.connect(self._open_preview_folder)
        self.btn_report.clicked.connect(self._export_debug)
        self.btn_save_project.clicked.connect(self._save_project_dialog)
        self.btn_load_project.clicked.connect(self._load_project_dialog)
        self.btn_rerun_step.clicked.connect(self._rerun_current_step)
        self.btn_clear_cache.clicked.connect(self._clear_project_cache)
        self.btn_export_final.clicked.connect(self._export_final_by_goal)
        self.btn_open_output.clicked.connect(lambda: QDesktopServices.openUrl(QUrl.fromLocalFile(str(Path(self.out_dir.text().strip()).resolve()))))
        ...
        self.btn_auto_diarization.clicked.connect(self._run_auto_diarization)
        self.speaker_mode.currentTextChanged.connect(self._on_speaker_mode_changed)
        self.goal.currentTextChanged.connect(self._update_button_states)
        self.subtitle_strategy.currentTextChanged.connect(lambda t: self.state.__setitem__("subtitle_strategy_user_override", t))

    def _load_tts_voices(self) -> None:
        voices = list_voices()
        if len(voices) <= 2:
            self._log("[WARNING] Không thể tải danh sách giọng trực tuyến. Đang dùng danh sách giọng mặc định.")
    ...
    def _generate_tts_segments(self) -> None:
        if not self.state.get("has_translation"):
            self._log("[WARNING] Chưa có bản dịch tiếng Việt. Vui lòng dịch trước khi tạo giọng đọc.")
            return
        ...
        try:
            ...
            self._progress_callback({"phase":"tts","percent":0,"message":"Đang chuẩn bị tạo giọng đọc..."})
            self._log("[INFO] Đang chuẩn bị tạo giọng đọc...")
            self._apply_speaker_table_to_segments()
            total = len(self.current_segments)
            for i in range(total):
                self._progress_callback({"phase":"tts","percent":int((i+1)*100/max(1,total)),"message":f"Đang tạo giọng đọc đoạn {i+1}/{total}...","current":i+1,"total":total})
            ...
            self._update_button_states()
            self._refresh_export_summary()
            created = sum(1 for x in self.current_segments if (x.get("tts_status") == "Đã tạo giọng"))
            for idx, x in enumerate(self.current_segments, start=1):
                if x.get("tts_status") == "Bỏ qua, chưa có bản dịch":
                    self._log(f"[WARNING] Đoạn {idx} chưa có bản dịch tiếng Việt, bỏ qua.")
                if str(x.get("tts_status", "")).startswith("Lỗi:"):
                    self._log(f"[ERROR] Lỗi khi tạo giọng đọc đoạn {idx}.")
            self.state["tts_stale"] = False
            self._progress_callback({"phase":"tts","percent":100,"message":"Hoàn tất tác vụ."})
            self._log(f"[OK] Đã tạo giọng đọc cho {created} đoạn.")
            ...
            self._log("[ERROR] Không thể tạo giọng nghe thử.")
    ...
    def _on_table_cell_changed(self, row: int, col: int) -> None:
        ...
        if col == 2:
            ...
            if it:
                ...
                self.state["tts_stale"] = True if self.state.get("has_tts") else False
                if self.state.get("tts_stale"):
                    self._log("[WARNING] Cấu hình giọng đã thay đổi, cần tạo lại TTS.")
                    self.state["has_tts"] = False
                    self._update_button_states()
                self._refresh_export_summary()
                self._log("[INFO] Đang cập nhật danh sách người nói...")
                self._log(f"[OK] Đã phát hiện {len(self.speaker_profiles)} người nói từ dữ liệu hiện tại.")
    ...
    def _save_project_json(self) -> str:
        ...
        fp = out / "project.json"
        st = ProjectState(
            input_video_path=self.selected_video_path,
            output_folder=self.out_dir.text().strip(),
            video_metadata=getattr(self, "video_meta", {}),
            selected_audio_track=self.audio_source.currentText() if hasattr(self, "audio_source") else "auto",
            source_language=self.source_lang.currentText(),
            target_language="vi",
            output_goal=self.goal.currentText(),
            transcript_segments=self.current_segments,
            translations=self.current_segments,
            speaker_profiles=self.speaker_profiles,
            subtitle_settings=self._subtitle_style_settings() if hasattr(self, "sub_font") else {},
            subtitle_detection_result={
                "detected": self.state.get("source_subtitle_detected"),
                "position": self.state.get("source_subtitle_position"),
                "suggested": self.state.get("subtitle_strategy_suggested"),
            },
            tts_settings={"engine": self.tts_engine.currentText(), "default_voice": self.default_voice.currentText(), "speed": self.tts_speed.value(), "pitch": self.tts_pitch.value(), "volume": self.tts_volume.value()},
            tts_manifest=str(out / "tts_manifest.json"),
            sync_settings={"sync_mode": self.sync_mode.currentText(), "safety_gap": self.safety_gap.value(), "audio_mode": self.audio_mode.currentText(), "source_audio_volume": self.source_audio_volume.value(), "vi_audio_volume": self.vi_audio_volume.value()},
            preview_files=[self.state.get("last_preview_path", "")] if self.state.get("last_preview_path") else [],
            final_export_paths=[str(out / x) for x in ["final_subtitled_video.mp4", "final_dubbed_video.mp4", "final_full_vietnamese_video.mp4"] if (out / x).exists()],
            cache_status=self.cache_status,
            workflow_state=self.state,
        )
        save_project_state(fp, st)
        (out / "speaker_profiles.json").write_text(json.dumps(self.speaker_profiles, ensure_ascii=False, indent=2), encoding="utf-8")
        self._log("[OK] Đã lưu dự án.")
        return str(fp)

    def _save_project_dialog(self) -> None:
        default_fp = str(Path(self.out_dir.text().strip()) / "project.json")
        fp, _ = QFileDialog.getSaveFileName(self, "Lưu dự án", default_fp, "JSON (*.json)")
        if not fp:
            return
        save_project_state(fp, load_project_state(self._save_project_json()))
        self._log("[OK] Đã lưu dự án.")

    def _load_project_dialog(self) -> None:
        fp, _ = QFileDialog.getOpenFileName(self, "Mở dự án", self.out_dir.text().strip(), "JSON (*.json)")
        if not fp:
            return
        self._load_project(fp)

    def _load_project(self, fp: str) -> None:
        payload = load_project_state(fp)
        self.selected_video_path = payload.get("input_video_path", "")
        self.input_path.setText(self.selected_video_path or "--")
        self.file_name.setText(Path(self.selected_video_path).name if self.selected_video_path else "Chưa chọn video")
        self.out_dir.setText(payload.get("output_folder", self.out_dir.text().strip()))
        self.source_lang.setCurrentText(payload.get("source_language", "auto"))
        self.goal.setCurrentText(payload.get("output_goal", self.goal.currentText()))
        self.current_segments = payload.get("transcript_segments", [])
        self.speaker_profiles = payload.get("speaker_profiles", {})
        self.cache_status = payload.get("cache_status", {})
        self.state.update(payload.get("workflow_state", {}))
        self._fill_table(self.current_segments)
        self._fill_speaker_panel()
        self._update_button_states()
        self._refresh_export_summary()
        miss = missing_cache_files(payload)
        if miss:
            self._log("[WARNING] Một số file cache không còn tồn tại. Bạn có thể xử lý lại bước tương ứng.")
        self._log("[OK] Đã tải dự án.")
    ...
    def _update_button_states(self) -> None:
        ...
        has_tts = bool(self.state.get("has_tts")) and not bool(self.state.get("tts_stale"))
        ...
        has_vi = any(((seg.get("vi_dubbing_text") or seg.get("vi_subtitle_text") or "").strip() for seg in self.current_segments))
        self.btn_generate_tts.setEnabled(has_translation and has_vi)
        ...
        self.btn_export_sub.setToolTip("Chưa có bản dịch tiếng Việt" if not has_translation else "")
        self.btn_generate_tts.setToolTip("Chưa có bản dịch tiếng Việt. Vui lòng dịch trước khi tạo giọng đọc." if (not has_translation or not has_vi) else "")
    ...
    def _progress_callback(self, info: dict) -> None:
        ...
        self.progress_task.setText(f"Tác vụ: {phase}")
        self.progress_detail.setText(msg if msg else (f"{cur}/{total}" if total else "--"))
        self.progress_elapsed.setText(f"Thời gian: {int(time.time()-getattr(self, '_progress_started_at', time.time()))}s")
        self._log(f"[INFO] Tiến độ: {pct}%")

    def _on_speaker_mode_changed(self) -> None:
        if self.speaker_mode.currentText() == "Gán thủ công, khuyên dùng":
            self.speaker_status.setText("Gán thủ công đang được dùng.")
        else:
            self.speaker_status.setText("Tách tự động cần mô hình diarization tùy chọn.")

    def _run_auto_diarization(self) -> None:
        if not self.current_segments:
            self._log("[ERROR] Chưa có dữ liệu lời thoại.")
            return
        self._progress_started_at = time.time()
        self._progress_callback({"phase":"diarization","percent":5,"message":"Đang chuẩn bị tách người nói..."})
        self._log("[INFO] Đang tách người nói tự động...")
        provider = PyannoteDiarizationProvider() if self.speaker_mode.currentText() == "Tách tự động nếu khả dụng" else ManualDiarizationProvider()
        res = provider.diarize("", {}, progress_callback=self._progress_callback)
        if not res.get("success"):
            self.speaker_status.setText("Chưa cấu hình mô hình tách người nói tự động.")
            self._log("[WARNING] Chưa cấu hình mô hình tách người nói tự động.")
            self._log(res.get("error", ""))
            return
        mapped = map_diarization_to_transcript(self.current_segments, res.get("segments", [])) if res.get("segments") else self.current_segments
        self._progress_callback({"phase":"diarization","percent":70,"message":"Đang gán người nói vào lời thoại..."})
        self.current_segments = mapped
        self._fill_table(self.current_segments)
        self.speaker_profiles = build_speaker_profiles(self.current_segments, self.speaker_profiles)
        self._fill_speaker_panel()
        fp = save_diarization_output(self.out_dir.text().strip(), res, mapped)
        self._progress_callback({"phase":"diarization","percent":100,"message":"Đã cập nhật người nói."})
        self._log(f"[OK] Đã tách được {len(self.speaker_profiles)} người nói.")
        self._log("[WARNING] Tách người nói tự động có thể chưa chính xác.")
        self._log("Đã tách người nói tự động. Vui lòng kiểm tra lại trước khi tạo giọng đọc.")
        self._log(f"Đã lưu kết quả tách người nói: {fp}")

    def _refresh_export_summary(self) -> None:
        segs = len(self.current_segments)
        spk = len(self.speaker_profiles)
        self.export_summary.setText(
            f"Video đầu vào: {Path(self.selected_video_path).name if self.selected_video_path else '--'}\n"
            f"Mục tiêu đầu ra: {self.goal.currentText()}\nNgôn ngữ nguồn: {self.source_lang.currentText()} | Ngôn ngữ đích: vi\n"
            f"Số đoạn lời thoại: {segs} | Số người nói: {spk}\n"
            f"Phụ đề: {'có' if self.state.get('has_subtitle') else 'chưa'} | Giọng đọc: {'có' if self.state.get('has_tts') else 'chưa'} | Preview: {'có' if self.state.get('has_preview') else 'chưa'}\n"
            f"Thư mục xuất: {self.out_dir.text().strip()}"
        )

    def _export_final_by_goal(self) -> None:
        self._log("[INFO] Bắt đầu xuất file cuối...")
        self._log(f"[INFO] Mục tiêu đầu ra: {self.goal.currentText()}")
        res = export_final_project(
            self.state,
            self.goal.currentText(),
            {"output_dir": self.out_dir.text().strip(), "input_video_path": self.selected_video_path},
            progress_callback=self._progress_callback,
        )
        if not res.get("success"):
            self._log(res.get("error", "[ERROR] Không thể xuất file."))
            return
        self._log("[OK] Đã xuất file thành công.")
    ...
    def _auto_analyze(self) -> None:
        ...
        try:
            ...
            self.cache_status["extracted_audio"] = str(out_audio)
            cached_transcript = Path(self.out_dir.text().strip()) / "transcript_source.json"
            if cached_transcript.exists():
                self._log("[INFO] Đang dùng dữ liệu cache.")
                payload = json.loads(cached_transcript.read_text(encoding="utf-8"))
                self.current_segments = ensure_speakers(payload.get("segments", []))
                detected_lang = payload.get("detected_language", "unknown")
            else:
    ...
    def _parse_hhmmss(self, text: str) -> float:
        try:
    ...
    def _open_preview_file(self) -> None:
        fp = self.state.get("last_preview_path", "")
        if fp and Path(fp).exists():
            QDesktopServices.openUrl(QUrl.fromLocalFile(str(Path(fp).resolve())))

    def _open_preview_folder(self) -> None:
        d = Path(self.out_dir.text().strip()) / "preview"
        d.mkdir(parents=True, exist_ok=True)
        QDesktopServices.openUrl(QUrl.fromLocalFile(str(d.resolve())))

    def _create_preview_clip(self) -> None:
        if not self.selected_video_path:
            self._log("[ERROR] Chưa có video đầu vào.")
            return
        self._log("[INFO] Đang tạo preview...")
        out_dir = Path(self.out_dir.text().strip()) / "preview"
        ptype = self.preview_type.currentText()
        out_name = "preview_subtitle.mp4" if ptype == "Preview phụ đề" else ("preview_dubbed.mp4" if ptype == "Preview lồng tiếng" else "preview_full.mp4")
        res = create_preview(
            self.selected_video_path,
            str(out_dir / out_name),
            self._parse_hhmmss(self.preview_start.text()),
            float(self.preview_duration.value()),
            ptype,
            str(Path(self.out_dir.text().strip()) / "translated_vi.ass"),
            str(Path(self.out_dir.text().strip()) / "final_vietnamese_voice.wav"),
            self.subtitle_strategy.currentText(),
            {"cover_color": self.cover_color.text().strip(), "height_pct": self.cover_height.value()/1000.0},
            {},
            progress_callback=self._progress_callback,
        )
        if not res.get("success"):
            self._log("[WARNING] Thiếu phụ đề hoặc giọng đọc cho loại preview đã chọn." if "Chưa có" in str(res.get("error","")) else "[ERROR] Không thể tạo preview.")
            self.logger.error("preview error: %s", res.get("error", ""))
            return
        self.state["has_preview"] = True
        self.state["last_preview_path"] = res.get("output_path", "")
        self.btn_open_preview_file.setEnabled(True)
        self._log(f"[OK] Đã tạo preview. {res.get('output_path','')}")

    def _open_sample_frames(self) -> None:
        folder = Path(self.out_dir.text().strip()) / "subtitle_detection_frames"
        if not folder.exists():
            self._log("[WARNING] Chưa có ảnh mẫu để mở.")
            return
    ...
    def _detect_source_subtitle(self) -> None:
        ...
        try:
            ...
            self._log("[INFO] Đang trích ảnh mẫu từ video...")
            dur = max(1.0, float(self.file_duration.text().replace(":", "").isdigit() and 0 or 1.0)) if self.file_duration.text() else 1.0
            # lấy thời lượng bằng ffprobe cho chính xác
            try:
                dur = float(subprocess.check_output(["ffprobe", "-v", "error", "-show_entries", "format=duration", "-of", "default=noprint_wrappers=1:nokey=1", self.selected_video_path], text=True).strip())
            except Exception:
                pass
            self._log("[INFO] Đang phân tích vùng phụ đề...")
            result = detect_burned_in_subtitles(self.selected_video_path, dur, self.out_dir.text().strip())
            has_sub = result.get("has_source_subtitle")
            pos = result.get("probable_position", "unknown")
            conf = float(result.get("confidence", 0.0))
            suggest = result.get("suggested_strategy", "Thêm phụ đề Việt ở dưới")
            frames = result.get("sample_frames", [])
            if has_sub is True and pos == "bottom":
                self._log("[OK] Có thể video có phụ đề ở phía dưới.")
            elif has_sub is True and pos == "top":
                self._log("[OK] Có thể video có phụ đề ở phía trên.")
            elif has_sub is False:
                self._log("[OK] Không phát hiện phụ đề gốc rõ ràng.")
            else:
            ...
            self.state["has_subtitle_detection"] = True
            self.state["source_subtitle_detected"] = has_sub
            self.state["source_subtitle_position"] = pos
            self.state["subtitle_strategy_suggested"] = suggest
            self._set_step_status(2, "Hoàn tất")
    ...
    def _preview_default_voice(self) -> None:
        try:
            text = self.test_tts_text.text().strip() if hasattr(self, "test_tts_text") else "Xin chào, đây là giọng đọc thử."
            self._log("[INFO] Đang tạo file nghe thử giọng...")
            sel = self.speaker_table.currentRow()
            if sel >= 0 and self.speaker_table.item(sel,0):
                spk = self.speaker_table.item(sel,0).text()
                voice_id = self.speaker_table.item(sel,5).text() if self.speaker_table.item(sel,5) else (self.default_voice.currentData() or VOICE_FEMALE)
                speed = int(self.speaker_table.item(sel,6).text() if self.speaker_table.item(sel,6) else "0")
                pitch = int(self.speaker_table.item(sel,7).text() if self.speaker_table.item(sel,7) else "0")
                volume = int(self.speaker_table.item(sel,8).text() if self.speaker_table.item(sel,8) else "0")
                out = str(Path("temp") / "speaker_voice_preview" / f"{spk}.mp3")
                sample_rows = [x for x in self.current_segments if x.get("speaker")==spk and (x.get("vi_dubbing_text") or x.get("vi_subtitle_text"))]
                if sample_rows:
                    text = (sample_rows[0].get("vi_dubbing_text") or sample_rows[0].get("vi_subtitle_text") or text)
            else:
                out = str(Path("temp") / "preview_voice.mp3")
                voice_id = self.default_voice.currentData() or self.default_voice.currentText()
                speed = self.tts_speed.value(); pitch = self.tts_pitch.value(); volume = self.tts_volume.value()
            synthesize_text(text, out, voice_id, speed, pitch, volume)
            self._log(f"[OK] Đã tạo file nghe thử giọng. {out}")
            QDesktopServices.openUrl(QUrl.fromLocalFile(str(Path(out).resolve())))
        except Exception:
            self._log("[ERROR] Không thể tạo giọng nghe thử.")
            self._log("[WARNING] Edge TTS cần kết nối internet.")
    ...
    def _apply_speaker_table_to_segments(self) -> None:
        ...
        if self.state.get("has_tts"):
            self.state["has_tts"] = False
            self.state["tts_stale"] = True
            self._log("[WARNING] Cấu hình giọng đã thay đổi, cần tạo lại TTS.")
            self._update_button_states()
        self._refresh_export_summary()
    ...
    def _translate_all(self) -> None:
        ...
        vi_cache = Path(self.out_dir.text().strip()) / "transcript_vi.json"
        if vi_cache.exists():
            self._log("[INFO] Đang dùng dữ liệu cache.")
            rows = json.loads(vi_cache.read_text(encoding="utf-8")).get("segments", self.current_segments)
            warning = ""
        else:
    ...
    def _export_subtitles(self) -> None:
        ...
        try:
            ...
            srt_path = generate_srt(self.current_segments, str(out_dir / "translated_vi.srt"))
            self._log("[INFO] Đang tạo phụ đề ASS...")
            style = self._subtitle_style_settings()
            ass_path = generate_ass(self.current_segments, str(out_dir / "translated_vi.ass"), style)
            ...
            self.subtitle_preview.setText(f"Đây là ví dụ phụ đề tiếng Việt.\nCỡ chữ: {style.get('font_size', 36)} | Vị trí: {self.sub_position.currentText()}")
            self._log(f"[OK] Đã tạo phụ đề thành công.\n- {srt_path}\n- {ass_path}\n- {settings_path}")
        except Exception as e:
            msg = str(e)
            if "Chưa có bản dịch tiếng Việt" in msg:
                self._log("[ERROR] Chưa có bản dịch tiếng Việt.")
            else:
                self._log("[ERROR] Lỗi khi tạo phụ đề.")
            self.logger.exception("subtitle export error: %s", e)
    ...
    def _render_subtitled_video(self) -> None:
        ...
        try:
            ...
            if not ass_path.exists():
                ...
                return
            self._log("[INFO] Đang chuẩn bị kết xuất video phụ đề...")
            self._log("[INFO] Đang kiểm tra file phụ đề ASS...")
            ...
            result = render_subtitled_video(
                self.selected_video_path,
                str(ass_path),
                str(out_dir / "final_subtitled_video.mp4"),
                strategy,
                cover,
                video_metadata={
                    "source_subtitle_position": self.state.get("source_subtitle_position", "unknown"),
                    "subtitle_strategy_suggested": self.state.get("subtitle_strategy_suggested", "Thêm phụ đề Việt ở dưới"),
                },
            )
            if result.get("skipped"):
                self._log("[INFO] Bạn đã chọn chỉ tạo file phụ đề rời. Không cần kết xuất video.")
                return
            if not result.get("success"):
                self.logger.error("render ffmpeg stderr: %s", result.get("error", ""))
                self._log("[ERROR] Lỗi khi kết xuất video.")
                return
            self.logger.info("ffmpeg cmd render: %s", result.get("ffmpeg_command", ""))
            settings_path = save_render_settings(str(out_dir), {
            ...
            out_video = result.get("output_path", "")
    ...
    def _rerun_current_step(self) -> None:
        row = self.steps.currentRow()
        if row == 2:
            self._log("[WARNING] Dữ liệu cache đã cũ, cần xử lý lại.")
            self._auto_analyze()
        elif row == 3:
            self._log("[WARNING] Dữ liệu cache đã cũ, cần xử lý lại.")
            self._translate_all()
        elif row == 4:
            self._create_preview_clip()
        elif row == 5:
            self._export_final_by_goal()
        else:
            self._log("[INFO] Bước hiện tại không cần làm lại.")

    def _clear_project_cache(self) -> None:
        out = Path(self.out_dir.text().strip())
        targets = [
            out / "transcript_source.json", out / "transcript_vi.json", out / "translated_vi.srt", out / "translated_vi.ass",
            out / "subtitle_detection.json", out / "tts_manifest.json", out / "final_vietnamese_voice.wav", out / "sync_report.json",
            out / "final_subtitled_video.mp4", out / "final_dubbed_video.mp4", out / "final_full_vietnamese_video.mp4",
        ]
        for fp in targets:
            if fp.exists():
                fp.unlink()
        if (out / "tts_segments").exists():
            shutil.rmtree(out / "tts_segments", ignore_errors=True)
        if (out / "preview").exists():
            shutil.rmtree(out / "preview", ignore_errors=True)
        self.cache_status = {}
        self._log("[OK] Đã xóa cache dự án.")
