    ...
    try:
        ...
        for i in range(0, len(segments), batch_size):
            ...
            for seg, vi, ph in zip(batch, outputs, protections):
                vi_full = _restore_terms(vi.strip(), ph)
                vi_short = vi_full if translation_mode == "full" else vi_full[:120].rstrip() + ("..." if len(vi_full) > 120 else "")
                row = dict(seg)
        ...
        for seg in segments:
            ...
            row["vi_subtitle_text"] = src
            row["vi_dubbing_text"] = src if translation_mode == "full" else src[:120]
            translated.append(row)
