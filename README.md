...
### Ghi chú cập nhật Phase 5 (ổn định)
- Khi tạo phụ đề, ứng dụng chỉ dùng `vi_subtitle_text` (không dùng `source_text` làm phụ đề tiếng Việt).
- Nếu chưa có bản dịch tiếng Việt, ứng dụng báo lỗi rõ ràng và không tạo file phụ đề.
- Nếu file `translated_vi.srt`/`translated_vi.ass` đã tồn tại, ứng dụng tự tạo tên có timestamp để tránh ghi đè im lặng.

### Cập nhật Phase 6 - Phát hiện phụ đề gốc
- Bấm **Kiểm tra phụ đề gốc** để trích ảnh mẫu tại 10%, 25%, 50%, 75%, 90% thời lượng video.
- Ứng dụng dùng heuristic (không OCR nặng) để ước lượng phụ đề gốc nằm trên/dưới.
- Kết quả lưu tại `output/subtitle_detection.json` và ảnh mẫu tại `output/subtitle_detection_frames/`.
- Có thể bấm **Mở ảnh mẫu** để kiểm tra thủ công khi kết quả không chắc chắn.
- Ở chế độ **Tự động, khuyên dùng**, ứng dụng gợi ý chiến lược phụ đề Việt phù hợp; người dùng luôn có thể chỉnh tay.

### Phụ thuộc bổ sung cho Phase 6
- `Pillow` được dùng để phân tích ảnh mẫu trong chức năng phát hiện phụ đề gốc (`subtitle_detection.py`).

### Cập nhật Phase 7 - Kết xuất video phụ đề
- Bấm **Kết xuất video phụ đề** sau khi đã có file ASS.
- Nếu chọn **Chỉ tạo file phụ đề rời**, ứng dụng sẽ không render video.
- Chế độ **Che phụ đề gốc và thay bằng phụ đề Việt** dùng `drawbox` trước khi burn ASS.
- File xuất chính: `output/final_subtitled_video.mp4` (tự thêm timestamp nếu trùng tên).

#### Lỗi thường gặp
- `ffmpeg` không tìm thấy: cần cài ffmpeg và thêm vào PATH.
- Phụ đề không hiện: kiểm tra file ASS và đường dẫn chứa ký tự đặc biệt.
- Lỗi font tiếng Việt: chọn font hỗ trợ tiếng Việt (Arial/Segoe UI).
- Đường dẫn có khoảng trắng/ký tự Unicode: ứng dụng đã escape cho ffmpeg nhưng vẫn nên dùng đường dẫn ngắn gọn nếu gặp lỗi môi trường.

### Cập nhật Phase 8 - Tạo giọng đọc tiếng Việt
- Chọn **Giọng đọc mặc định** (Hoài My / Nam Minh), điều chỉnh tốc độ, cao độ, âm lượng.
- Bấm **Nghe thử giọng** để tạo `temp/preview_voice.mp3`.
- Bấm **Tạo giọng đọc** để tạo từng file đoạn trong `temp/tts_segments/`.
- Manifest chi tiết lưu tại `output/tts_manifest.json` (UTF-8).
- TTS chỉ dùng `vi_dubbing_text` hoặc `vi_subtitle_text`, không dùng `source_text`.
- Lưu ý: Edge TTS cần internet; nếu lỗi mạng sẽ báo cảnh báo tiếng Việt.

### Cập nhật Phase 9 - Đồng bộ & lồng tiếng
- Bấm **Xuất lồng tiếng** sau khi đã có `output/tts_manifest.json`.
- Ứng dụng đặt từng đoạn TTS theo timestamp transcript bằng `adelay`, không nối chuỗi trực tiếp.
- Chống đè tiếng bằng tăng tốc (`atempo`) và cắt ngắn khi cần.
- Chế độ âm thanh gốc:
  - Giữ âm thanh gốc
  - Giảm âm lượng âm thanh gốc
  - Tắt âm thanh gốc
...
### Cập nhật Phase 10 - Đa người nói & tiến độ xử lý
- Có thể gán người nói thủ công trực tiếp trong cột **Người nói** (SPEAKER_01..SPEAKER_05 hoặc nhãn tùy chỉnh).
- Bảng người nói cho phép đổi tên hiển thị, gán giọng, tốc độ, cao độ, âm lượng.
- Nút **Tự gán giọng thông minh** hỗ trợ phân bổ giọng theo số lượng người nói.
- Có thể chọn từng người nói và bấm **Nghe thử giọng** để tạo preview riêng trong `temp/speaker_voice_preview/`.
- Khi đổi cấu hình giọng sau khi đã tạo TTS, ứng dụng đánh dấu TTS cũ và yêu cầu tạo lại.
- Thanh tiến độ hiển thị % và trạng thái cho các tác vụ dài (đặc biệt TTS, đồng bộ, kết xuất).
- Tách người nói tự động hiện là tùy chọn placeholder và có thể cần mô hình riêng.

### Cập nhật Phase 11 - Tách người nói tự động (tùy chọn)
- Mặc định khuyến nghị: **Gán thủ công, khuyên dùng**.
- Tách tự động là tính năng tùy chọn, có thể cần `pyannote.audio` + mô hình/token phù hợp.
- Nếu chưa cài/cấu hình, ứng dụng sẽ hiển thị hướng dẫn thân thiện và không bị crash.
- Sau khi tách tự động, luôn cần kiểm tra/sửa lại nhãn người nói thủ công trước khi tạo giọng đọc.

### Cập nhật Phase 12 - Preview trước khi xuất
- Chọn thời điểm bắt đầu và thời lượng (khuyến nghị 10-15 giây) rồi bấm **Tạo preview**.
- Có 3 loại preview:
  - Preview phụ đề
  - Preview lồng tiếng
  - Preview phụ đề + lồng tiếng
- File preview lưu ở `output/preview/`.
- Nếu preview chưa ổn, chỉnh lại phụ đề/giọng đọc rồi tạo preview lại trước khi xuất toàn bộ video.

## Phase 14 - Lưu/Mở dự án, cache và tiếp tục xử lý
- Nút mới trong phần **Xuất file**:
  - **Lưu dự án**: lưu trạng thái vào `output/project.json` (hoặc vị trí bạn chọn).
  - **Mở dự án**: nạp lại trạng thái và tiếp tục xử lý từ bước dang dở.
  - **Làm lại bước này**: chạy lại nhanh bước hiện tại khi cache đã cũ.
  - **Xóa cache dự án**: xóa các tệp trung gian để làm sạch dự án.
- Ứng dụng cache các tệp: audio tách, transcript, bản dịch, SRT/ASS, subtitle detection, tệp TTS, manifest, sync report, preview và file xuất cuối.
- Khi dữ liệu đầu vào/cài đặt không đổi, ứng dụng ưu tiên dùng cache và ghi log:
  - `[INFO] Đang dùng dữ liệu cache.`
- Khi cache không còn hợp lệ hoặc bị thiếu:
  - `[WARNING] Dữ liệu cache đã cũ, cần xử lý lại.`
  - `Một số file cache không còn tồn tại. Bạn có thể xử lý lại bước tương ứng.`
- Khi chỉnh cấu hình giọng hoặc phụ đề, các bước sau sẽ được đánh dấu cần làm lại để tránh xuất sai kết quả.
